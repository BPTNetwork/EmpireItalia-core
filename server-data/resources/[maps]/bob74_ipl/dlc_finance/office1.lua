...
FinanceOffice1 = {
	...
	Safe = {
		...
		Open = function(door)
			if door:lower() == "left" then FinanceOffice1.Safe.isLeftDoorOpen = true
			elseif door:lower() == "right" then FinanceOffice1.Safe.isRightDoorOpen = true
			else print("[bob74_ipl] Warning: Invalid safe door - use 'left' or 'right'") end
		end,

		Close = function(door)
			if door:lower() == "left" then FinanceOffice1.Safe.isLeftDoorOpen = false
			elseif door:lower() == "right" then FinanceOffice1.Safe.isRightDoorOpen = false
			else print("[bob74_ipl] Warning: Invalid safe door - use 'left' or 'right'") end
		end,

		SetDoorState = function(door, open)
			local heading = FinanceOffice1.Safe.doorHeadingL
			local handle = door:lower() == "left"
				and FinanceOffice1.Safe.GetDoorHandle(FinanceOffice1.currentSafeDoors.hashL)
				or FinanceOffice1.Safe.GetDoorHandle(FinanceOffice1.currentSafeDoors.hashR)

			heading = heading - (door:lower() == "left" and 90 or 180) + (open and 90 or 0)
			if handle == 0 then return print("[bob74_ipl] Warning: safe door handle is 0") end
		...
		GetDoorHandle = function(hash)
			...
			repeat
				...
				if handle == 0 then Citizen.Wait(25); timeout = timeout - 1 end
